...
Assuming that a swarm has been deployed with a fixed set of manager and worker nodes, there might be a need for scaling the swarm either up or down in size. For this use-case the scaling is focussed on the availability of nodes running with qix engine containers, hence we should scale worker nodes. New worker nodes that are joining the swarm will spin up a qix engine automatically, due to global mode set on qix-engine service in [docker-compose.yml](../docker-compose.yml).
